				    ...
				    struct snd_soc_dai *dai)
{
	...
	iec.status[0] |= 0; /* replace with !professional define*/
	/* here fill lpcm audio (0) */
	iec.status[0] |= 0; /* replace with !non-audio define*/
	/* here fill copyright info */
	iec.status[0] |= IEC958_AES0_CON_NOT_COPYRIGHT;
	/* here fill emphasis info */
	iec.status[0] |= IEC958_AES0_CON_EMPHASIS_NONE;
	/* here fill mode info */
	iec.status[0] |= 0; /* replace with mode = 0 define*/
...
static int omap_hdmi_dai_shutdown(struct snd_pcm_substream *substream,
				struct snd_soc_dai *dai)
{
	return hdmi.dssdev->driver->audio_enable(hdmi.dssdev, false);
}

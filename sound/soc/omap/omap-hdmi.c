				  ...
				  struct snd_soc_dai *dai)
{
	...
	if (!hdmi.dssdev->driver->audio_supported(hdmi.dssdev))
		return -ENODEV;
...
static int omap_hdmi_dai_prepare(struct snd_pcm_substream *substream,
				struct snd_soc_dai *dai)
{
	return hdmi.dssdev->driver->audio_enable(hdmi.dssdev);
}
				...
				struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		hdmi.dssdev->driver->audio_start(hdmi.dssdev);
		break;
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		hdmi.dssdev->driver->audio_stop(hdmi.dssdev);
		break;
	default:
		err = -EINVAL;
	}
...
static void omap_hdmi_dai_shutdown(struct snd_pcm_substream *substream,
				struct snd_soc_dai *dai)
{
	hdmi.dssdev->driver->audio_disable(hdmi.dssdev);
}

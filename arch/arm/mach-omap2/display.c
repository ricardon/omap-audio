	...
	/* Create devices for HDMI audio drivers */
		...
		if (dssdev->type != OMAP_DISPLAY_TYPE_HDMI)
                ...
                if (card_created == false) {
                        aud_pdev = create_simple_dss_pdev("omap-hdmi-audio-card",
                                                   -1, 0, dss_pdev);
                        if (IS_ERR(aud_pdev))
                        {
                                pr_err("Could not build platform_device for omap-hdmi-audio-card\n");
                                return PTR_ERR(aud_pdev);
                        }
                        card_created = true;
                }

                /* One device for each HDMI connector in the board */
                aud_pdev = create_simple_dss_pdev("hdmi-audio-codec",
                                           dssdev->dev.id, NULL, 0, dss_pdev);
                if (IS_ERR(aud_pdev))
                {
                        pr_err("Could not build platform_device for omap-hdmi-audio-card\n");
                        return PTR_ERR(aud_pdev);

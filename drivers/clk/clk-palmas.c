...
static int palmas_prepare_clk32kg(struct clk_hw *hw)
{
	...
	ret = palmas_clock_setbits(palmas_clk->palmas,
			PALMAS_CLK32KG_CTRL, CLK32KG_CTRL_MODE_ACTIVE);
	if (ret)
...
static int palmas_prepare_clk32kgaudio(struct clk_hw *hw)
{
	...
	ret = palmas_clock_setbits(palmas_clk->palmas,
		PALMAS_CLK32KGAUDIO_CTRL, CLK32KGAUDIO_CTRL_MODE_ACTIVE);
	if (ret)
...
static int __init palmas_clk_init(void)
{
	...
	ret = platform_driver_register(&palmas_clk_driver);
	if (ret != 0)
		pr_err("Failed to register PALMAS clock driver: %d\n", ret);

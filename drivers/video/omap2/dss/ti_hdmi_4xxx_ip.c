		...
		struct snd_aes_iec958 *iec, struct snd_cea_861_aud_if *aud_if)
{
	...
	/*
	 * Parse IEC60958 channel status word
	 */
	/* TODO: Better to use bit ops functions */
	...
	core.iec60958_cfg.word_max_length =
		iec->status[3] & IEC958_AES4_CON_MAX_WORDLEN_24;
	core.iec60958_cfg.word_length =
		(iec->status[3] & IEC958_AES4_CON_WORDLEN)
		>> IEC958_AES4_CON_WORDLEN_SHIFT;

	/*
	 * check if word lengt is 16-bit as several optimization can be
	...
	/* TODO: left justification only for 16-bit samples?*/
	if (word_length_16b)
		core.i2s_cfg.justification = HDMI_AUDIO_JUSTIFY_LEFT;
	else
		core.i2s_cfg.justification = HDMI_AUDIO_JUSTIFY_RIGHT;
	core.i2s_cfg.in_length_bits = core.iec60958_cfg.word_length;
	if (core.iec60958_cfg.word_max_length ==
		IEC958_AES4_CON_MAX_WORDLEN_24)
	...
	/*
	 * the HDMI IP need to enable four stereo channels when transmitting
	...
	/* check valid sample rates as indicated in HDMI 1.4a, table 7-4 */
	/* check dependency video (HDMI 1.4a, table 7-5) */

	/* move cts/n calculation here. no need to have in 4xxx ip. requires
	 * deep color api */
